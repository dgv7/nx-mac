...
on launchBaram(this_URL)
	-- Immediate user feedback (before wine cold-start ~50s)
	if this_URL is "" then
		display notification "바람의나라 로그인 창 준비 중 (약 50초)" with title "NX Launcher" subtitle "Plug 초기화 중"
	else
		-- Only show if Plug not already running (cold-start path)
		try
			do shell script "pgrep -f 'NexonPlug\\.exe' >/dev/null 2>&1 && pgrep -f 'wineserver' >/dev/null 2>&1"
		on error
			display notification "바람의나라 실행 준비 중 (약 50초)" with title "NX Launcher" subtitle "Plug 초기화 중"
		end try
	end if
